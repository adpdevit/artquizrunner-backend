...
public class GameEngineService {
	...
	public GameState evolveGameStatePositive(final GameState curentGameState) {
		return evolveGameState(curentGameState, gameParameters.getEvolPoints(), gameParameters.getEvolEndStatePositive(), 
				curentGameState.getGoodInARow() + 1L, 0L);
	}
	
	public GameState evolveGameStateNegative(final GameState curentGameState) {
		return evolveGameState(curentGameState, 0L, gameParameters.getEvolEndStateNegative(), 
				0L, curentGameState.getBadInARow() + 1L);
	}
	
	private GameState evolveGameState(final GameState currentGameState, final Long evolPoints, 
			final Long evolEndState, final Long goodInARow, final Long badInARow) {
